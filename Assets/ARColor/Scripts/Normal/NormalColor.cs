...
public class NormalColor : MonoBehaviour {
    ...
    public void Get_Position() {
        Fish.GetComponent<Renderer>().material.SetVector("_Uvpoint1", new Vector4(TopLeft_Pl_W.x, TopLeft_Pl_W.y, TopLeft_Pl_W.z, 1f));
        Fish.GetComponent<Renderer>().material.SetVector("_Uvpoint2", new Vector4(BottomLeft_Pl_W.x, BottomLeft_Pl_W.y, BottomLeft_Pl_W.z, 1f));
        Fish.GetComponent<Renderer>().material.SetVector("_Uvpoint3", new Vector4(TopRight_Pl_W.x, TopRight_Pl_W.y, TopRight_Pl_W.z, 1f));
        Fish.GetComponent<Renderer>().material.SetVector("_Uvpoint4", new Vector4(BottomRight_Pl_W.x, BottomRight_Pl_W.y, BottomRight_Pl_W.z, 1f));

       
        Matrix4x4 P = GL.GetGPUProjectionMatrix(Camera.main.projectionMatrix, BLrenderIntoTexture);
        Matrix4x4 V = Camera.main.worldToCameraMatrix;
        Matrix4x4 VP = P * V;
        Fish.GetComponent<Renderer>().material.SetMatrix("_VP", VP);      
    }
    ...
    IEnumerator ScreenShot()
    {
        ...
        Texture2D Te = new Texture2D(Screen.width, Screen.height, TextureFormat.RGB24, false);
            ...
            Te.Apply();
            Fish.GetComponent<Renderer>().material.mainTexture = Te;
     }

    public void RemoveTexture()
    {
        Fish.GetComponent<Renderer>().material.mainTexture = Te_Tran;
    }
